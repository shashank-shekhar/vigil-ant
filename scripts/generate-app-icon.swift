...
//
// Generates macOS app icon (bug-ant) at all required sizes
// for the AppIcon asset catalog.
//
...
let iconsetPath = "Vigilant/Vigilant/Assets.xcassets/AppIcon.appiconset"
...
func renderAppIcon(pixels: Int) -> NSImage {
    ...
    let image = NSImage(size: size, flipped: false) { rect in
        ...
        let cornerRadius = px * 0.22  // macOS icon corner radius
        let bgPath = NSBezierPath(roundedRect: rect, xRadius: cornerRadius, yRadius: cornerRadius)

        // Dark teal/green gradient background
        let colorTop = NSColor(red: 0.15, green: 0.45, blue: 0.42, alpha: 1.0)
        let colorBottom = NSColor(red: 0.10, green: 0.30, blue: 0.28, alpha: 1.0)
        if let gradient = NSGradient(starting: colorBottom, ending: colorTop) {
            gradient.draw(in: bgPath, angle: 90)
        }

        // Draw the bug-ant icon in white, centered (~75% of canvas)
        let iconSize = px * 0.75
        let iconX = (px - iconSize) / 2
        let iconY = (px - iconSize) / 2
...
for iconSize in sizes {
    let image = renderAppIcon(pixels: iconSize.pixels)
